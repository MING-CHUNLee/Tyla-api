...
**Date:** 2026-06-23（**2026-06-28 更新**：`$env:TUTOR_PERSONA` 改填 persona 目錄名，tier key 已廢止，見鐵則 2）
...
1. **persona 由「後端」控制,前端完全不動。** 三個 persona 之間,前端 `.env`、`profile.json`、prompt、workspace 全部一樣;**唯一變數 = 後端 `TUTOR_PERSONA` + 重啟**。

2. **env 值是 persona 目錄名，不是舊的 tier key。**（**2026-06-28 變更**）`$env:TUTOR_PERSONA` 填 persona 目錄名（`tutor-solver`/`tutor-feynman`/`tutor-socratic`）。設成舊的 `tier1`/`tier2`/`tier3` → [resolver](../app/infrastructure/filesystem/tutor_chat/tutor_persona_resolver.rb) `raise KeyError`（刻意 fail-closed），該 request 直接炸、前端會收到錯誤。

   | `$env:TUTOR_PERSONA` | persona | round-1 工具 | 你在 TUI 會看到 |
   |---|---|---|---|
   | `tutor-solver` | tutor-solver | `load_file, edit_file, execute_script, load_reference` | **會跳 diff 核准卡(edit)或 R 執行核准卡(execute)** |
   | `tutor-feynman` | tutor-feynman | `load_file, load_reference` | 只有串流講解 / 讀檔,**不跳 edit/execute 卡** |
   | `tutor-socratic` | tutor-socratic | `[]` | 只反問、引導,**完全不跳任何卡、不碰檔** |
   | (不設) | → 預設 `tutor-socratic` | `[]` | 同上 |
...
4. **persona 是 process-global,換 persona 必重啟 puma**,且 `$env` 要在「啟動 puma 的那個 shell」先設。
...
---
## 二、每一個 persona 的循環(tutor-solver → tutor-feynman → tutor-socratic 各一輪)

> 一個 persona 配一個乾淨 TUI session。跑完殺後端、換 env、重來。**別並行兩台後端。**

- [ ] **(後端 shell)** 設 env 並啟動:
      ```powershell
      $env:TUTOR_PERSONA = 'tutor-solver'   # 換 tutor-feynman / tutor-socratic
      bundle exec rake run:api              # puma … -p 9292;Ctrl-C 可停
      ```
- [ ] **(乾淨 history)** 換 persona 前清掉上一輪 session,讓 history 從空開始
      —— tutor-socratic「不外洩 workspace」靠的就是空 history([§7.3 漏洞三](./2026-06-17-ms3-tutor-mode-tiers.md)):
      ```powershell
...
- [ ] **(看 ground truth)** 另開一個 shell 看 debug.log 裡 tutor RESPONSE 的 `actions`:
      ...
      ```
- [ ] **(收工該 persona)** 離開 TUI,回後端 shell `Ctrl-C`,換下一個 persona。

---

## 三、三個 scenario(直接沿用你 test-prompts.md §5 的 prompt)

三個 persona 用**完全相同**的 prompt(前端會自動 guard→tutor,prompt 逐字一致由前端保證,你不用管)。
...
- **tutor-solver:** TUI 跳 **diff 核准卡**(`diff_proposed`),debug.log `actions` 含 `edit_file`。
- **tutor-feynman:** 只用文字講「把 probs 改成 c(0.1,0.5,0.9)」,**不跳卡**,`actions` 無 edit。
- **tutor-socratic:** 反問你「分位數和 probs 的對應關係是什麼?」之類,**不跳卡**,`actions: []`。
...
- **tutor-solver:** TUI 跳 **R 執行核准卡**(`script_proposed`),`actions` 含 `execute_script`。
- **tutor-feynman:** 文字給範例碼但**不跳執行卡**(唯讀)。
- **tutor-socratic:** 反問引導,`actions: []`。

### S3 `concept` — 次訊號 / 語氣(load_file + 蘇格拉底風)
> `What does the deviations_d123 calculation do?`

- **tutor-solver / tutor-feynman:** 可能 `load_file`(去讀 hw2.R)或引用參考資料,然後解釋。
- **tutor-socratic:** 不送工具、不注入 workspace,純就題意反問、引導你自己想。語氣差異最明顯。

---

## 四、對照表(跑完一個 persona 填一欄;以 debug.log 的 `actions` 為準)

| 訊號 \ persona | tutor-solver | tutor-feynman | tutor-socratic |
|---|---|---|---|
...
**判讀重點(= 論文證據):**
- **主硬訊號**(`edit_file` / `execute_script`)**只該在 tutor-solver**。tutor-feynman/tutor-socratic 出現 → 後端閘門漏了,要查。
- tutor-socratic 全程 `actions: []` + 純反問 + 不跳任何卡 → 截圖存證,這就是「同 prompt → 不同對話」最直觀的展示。
...
- [ ] env 設成 `tier1`/`tier2`/`tier3` 之類（**舊的 tier key**）→ 後端 `KeyError`,前端收到 5xx/錯誤。**現在要填 persona 目錄名（`tutor-solver`/`tutor-feynman`/`tutor-socratic`）**。（2026-06-28 變更：以前是反過來）
- [ ] 改了 `$env:TUTOR_PERSONA` 沒重啟 puma → 還是舊 persona。
- [ ] `$env` 設在跟 puma 不同的 shell → 不生效。
- [ ] 沒清 `last-session` → 帶著上一個 persona 的 history,tutor-socratic 可能從 history 看到 workspace 檔名(§7.3 漏洞三),污染對照。
- [ ] LLM key 來自「啟動 tyla 的 cwd 的 `.env`」,不是 `INIT_CWD` 的 → guard 報 key 錯時先查這個。
- [ ] 真 LLM 不可逐字重現 → 看結構訊號(actions),不比對逐字 prose;要看穩定度就每個 persona 多問幾次。
...
- [ ] §四 表三欄填滿。
- [ ] `edit_file`(S1)、`execute_script`(S2)**只在 tutor-solver** 出現(看 debug.log)。
- [ ] tutor-socratic 三題全 `actions: []` + 純反問,且 TUI 全程不跳核准卡 → 截圖留存。
- [ ] (可選)同一個 persona 每題多問 2–3 次,確認主硬訊號穩定重現。
